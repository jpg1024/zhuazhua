...
- **API Key 加密存储**：密钥经 Windows DPAPI（`CryptProtectData`）加密后写入 `config.json`（`dpapi:` 前缀密文），仅本机当前用户可解密；旧版明文密钥在下次保存时自动迁移为密文。
- **明文 HTTP 警告**：AI 地址为非本地的 `http://` 时（localhost / 127.0.0.1 除外），保存前会弹窗确认，避免 API Key 被中间人截获。
- **键盘监听边界**：打字巡逻与全局快捷键通过轮询 `GetAsyncKeyState` 实现，**只统计按键节奏（是否在打字 / 打字速度），不记录任何按键内容，也不落盘**。
- **数据全部本地**：配置、成长档案、成就、每日任务均存于 `%APPDATA%\zoo_desktop_pet\`，无遥测、无统计上报、无自动更新通道。
- **开机自启动**：仅写入当前用户注册表 `HKCU\...\Run`（值名 `zhuazhua`），无需管理员权限；关闭开关并保存即删除该项。
- **皮肤导入防护**：自定义图片 / OBJ 模型有大小与面数上限（防解码炸弹），mtllib 引用路径拒绝绝对路径、盘符与 `..` 穿越，复制范围限定在皮肤目录内。
